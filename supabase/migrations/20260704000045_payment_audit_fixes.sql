-- payment_audit_fixes
-- Corrige 2 problemas encontrados na auditoria enterprise de pagamentos:
--
-- FIX-1: execute_order_split_v2 usava subtotal (pré-desconto) como base do split,
--   creditando wallets a mais do que o valor recebido do Asaas quando havia cupons
--   ou desconto de pontos. Agora usa (total - delivery_fee), que reflete o valor
--   líquido de produtos efetivamente pago pelo comprador.
--
-- FIX-2: courier_earnings não tinha UNIQUE em delivery_id; retries do
--   updateDeliveryStatus() podiam inserir ganhos em dobro.

-- ── 1. UNIQUE em courier_earnings ────────────────────────────────────────────

ALTER TABLE public.courier_earnings
  ADD CONSTRAINT IF NOT EXISTS courier_earnings_delivery_uq UNIQUE (delivery_id);

-- ── 2. execute_order_split_v2 — usa total-delivery_fee como base do split ────
...
BEGIN
  -- Adquire row lock ANTES de checar idempotência (previne TOCTOU).
  ...
  -- FIX: usa (total - delivery_fee) como base de produtos, que já desconta
  -- cupons e pontos, garantindo que wallet_credits <= valor recebido do Asaas.
  v_products    := GREATEST(0, COALESCE(v_order.total, 0) - COALESCE(v_order.delivery_fee, 0));
