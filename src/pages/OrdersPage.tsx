...
import { ArrowLeft, Clock3, Package, ReceiptText, RefreshCw } from "lucide-react";
import { useNavigate } from "react-router-dom";

import { getOrderById, getProductImageUrl, type Order } from "../services/gizApi";
...
export default function OrdersPage() {
  ...
  async function loadOrders(silent = false) {
    ...
    try {
      ...
      const data = await Promise.all(ids.map(getOrderById));
      setOrders(data);
    } catch (e) {
      console.error(e);
    } finally {
          ...
          <div className="space-y-3">
            {[1, 2].map((i) => (
              <div key={i} className="rounded-3xl bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 h-16 rounded-xl bg-[#f1f5f9] animate-pulse" />
              </div>
            ))}
          </div>
        ) : orders.length === 0 ? (
    ...
    <div className="overflow-hidden rounded-3xl border border-[#e8eaf0] bg-white shadow-sm">
      ...
      <div className="bg-gradient-to-r from-[#7c3aed] to-[#2563eb] p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-[10px] font-black ${
        ...
        {open && (
          <div className="mt-3 space-y-2 border-t border-[#f1f5f9] pt-3">
            ...
            <div className="border-t border-[#f1f5f9] pt-3 space-y-1.5">
              ...
              <div className="flex justify-between text-xs text-[#64748b]">
                <span>Entrega</span>
                <strong>{formatBRL(order.deliveryFee)}</strong>
              </div>
